...
import {
  ...
  Eye, X, ThumbsUp, ThumbsDown, MessageCircle,
...
function CreatorLink({
  ...
  const handleTouch = (e: React.TouchEvent) => {
    e.stopPropagation(); // impede que o player intercete este toque
  };
  ...
  return (
    <span
      ...
      onTouchEnd={(e) => {
        e.stopPropagation();
        // navega programaticamente em vez de depender do Link nativo
        navigate(to);
      }}
      onKeyDown={(e) => {
...
export default function Video() {
  ...
  const fetchVideo = useCallback(async () => {
    ...
    setLoading(true);
    const videoQ = isUUID
      ? supabase.from("videos").select("id, slug, title, description, video_url, thumbnail_url, views, created_at, category, duration, user_id, tags").eq("id", slug).single()
      : supabase.from("videos").select("id, slug, title, description, video_url, thumbnail_url, views, created_at, category, duration, user_id, tags").eq("slug", slug).single();
    ...
    setVideoId(vid.id);
    supabase.rpc("increment_video_views", { video_id: vid.id }).then(() => {});
    ...
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="space-y-5 pt-2">
                ...
                {!currentUserId ? (
                  <button onClick={() => requireAuth("comment")} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 border border-white/10 hover:border-pink-500/30 hover:bg-pink-500/5 transition-all group">
      ...
      {showAuthPopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="relative max-w-md w-full bg-gradient-to-br from-[#1a0a2e] to-[#2d0a2e] rounded-2xl border border-pink-500/30 shadow-[0_0_50px_rgba(236,72,153,0.25)]">
            <button onClick={() => setShowAuthPopup(false)} className="absolute top-4 right-4 text-white/30 hover:text-white transition-colors">
              <X size={20} />
            </button>
            <div className="p-8 text-center">
              <div className="w-16 h-16 mx-auto mb-5 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 flex items-center justify-center">
                {pendingAction === "like" && <ThumbsUp size={26} className="text-white" />}
                {pendingAction === "dislike" && <ThumbsDown size={26} className="text-white" />}
                {pendingAction === "save" && <Bookmark size={26} className="text-white" />}
                {pendingAction === "subscribe" && <Users size={26} className="text-white" />}
