// src/pages/modelos/ModelosPage.tsx
// Lista pública de todos os criadores/modelos da plataforma
// Rota: /modelos
...
function CreatorCard({
  ...
  return (
    <div className="group relative bg-white/[0.03] border border-white/8 rounded-2xl overflow-hidden hover:border-white/16 hover:bg-white/[0.05] transition-all duration-300">
      ...
      {/* ── Faixa de preview com thumbnails reais ── */}
      <div
      ...
      <div className="px-4 pb-4">
        {/* Avatar (overlapping) + botão subscrever */}
...
export default function ModelosPage() {
  ...
  // Subscriptions do utilizador actual
  useEffect(() => {
  ...
  const handleSubscribe = async (creatorId: string) => {
    if (!currentUserId) { setPendingId(creatorId); setShowAuth(true); return; }
  ...
  return (
    <Layout>
      <div className="max-container safe-area py-8 space-y-8">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-5">
            ...
            <div className="flex gap-3 flex-shrink-0">
              {[
                ...
                { label: "Novos",       value: loading ? "—" : fmtNum(creators.filter(c => c.video_count === 0).length),          icon: Sparkles, color: "text-neon-purple" },
              ].map(({ label, value, icon: Icon, color }) => (
                <div key={label} className="text-center px-5 py-4 rounded-xl bg-white/5 border border-white/8">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 focus-within:border-neon-purple/35 transition-all">
            ...
            />
            {query && <button onClick={() => setQuery("")} className="text-foreground/28 hover:text-foreground/60"><X size={13} /></button>}
          </div>
        ...
        {loading ? (
              ...
              <div key={i} className="rounded-2xl bg-white/[0.03] border border-white/8 overflow-hidden animate-pulse">
                ...
                <div className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {[...Array(3)].map((_, j) => <div key={j} className="h-12 rounded-xl bg-white/5" />)}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <p className="text-foreground/28 text-sm">{dQuery ? "Tenta uma pesquisa diferente." : "Ainda não há criadores ativos."}</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {displayed.map((creator, i) => (
              <CreatorCard
                ...
                onOpenChannel={(id) => navigate(`/modelo/${id}`)}
              />
            ))}
          </div>
        )}
      ...
      {showAuth && <AuthPopup action="subscribe" onClose={() => setShowAuth(false)} />}
    </Layout>
  );
}