// src/pages/categoria/Japão.tsx
// Categoria: Japão — slug BD: "japão"
...
export default function JapaoPage() {
  useDocumentTitle({ title: "Vídeos do Japão - SuckOrSex" });
  ...
  const fetchData = useCallback(async () => {
    ...
    if (publishedVids.length === 0) {
      const info = `${vids.length} vídeo(s) com category~="japão" mas nenhum é published+public.\n${JSON.stringify(vids.map((v: any) => ({ status: v.status, visibility: v.visibility })), null, 2)}`;
      setDebugInfo((import.meta as any).env?.DEV ? info : null);
      setVideos([]);
      setLoading(false);
      return;
  ...
  useEffect(() => {
    const ch1 = supabase.channel("japão-videos-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "videos" },
        (payload) => {
          const rec = (payload.new ?? payload.old) as any;
          if (!rec?.category || (rec.category as string).toLowerCase().trim() === "japão") fetchData();
        }
      ).subscribe();

    const ch2 = supabase.channel("japão-likes-rt")
  ...
  return (
    <Layout>
      <div className="max-container safe-area py-8 space-y-8">
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-400/20 via-rose-700/10 to-transparent border border-red-400/25 p-8">
          <div className="absolute -top-10 -right-10 w-48 h-48 bg-red-400/10 rounded-full blur-3xl" />
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-400 to-rose-700 flex items-center justify-center shadow-lg">
                  <MapPin size={20} className="text-white" />
                </div>
                <h1 className="text-3xl font-black bg-gradient-to-r from-red-400 to-rose-700 bg-clip-text text-transparent">
                  Japão
                </h1>
              </div>
              <p className="text-foreground/60 text-sm max-w-md">O universo único do conteúdo japonês.</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              placeholder="Pesquisar em Japão..."
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-foreground/35 text-sm max-w-xs">
                Assim que um criador publicar um vídeo na categoria "Japão", aparece aqui automaticamente.
              </p>
            </div>
