// src/pages/categoria/CompilaçãoAuthenticated.tsx
// Categoria: Compilação — slug BD: "compilação"
...
import {
  LayoutGrid, Eye, Heart, Play, Search,
...
export default function CompilacaoAuthenticatedPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (publishedVids.length === 0) {
      const info = `${vids.length} vídeo(s) com category~="compilação" mas nenhum é published+public.\n${JSON.stringify(vids.map((v: any) => ({ status: v.status, visibility: v.visibility })), null, 2)}`;
      setDebugInfo((import.meta as any).env?.DEV ? info : null);
      setVideos([]);
      setLoading(false);
      return;
  ...
  useEffect(() => {
    const ch1 = supabase.channel("compilação-auth-videos-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "videos" },
        (payload) => {
          const rec = (payload.new ?? payload.old) as any;
          if (!rec?.category || (rec.category as string).toLowerCase().trim() === "compilação") fetchData();
        }
      ).subscribe();

    const ch2 = supabase.channel("compilação-auth-likes-rt")
  ...
  return (
    <LayoutAuthenticated>
      <div className="max-container safe-area py-8 space-y-8">
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-400/20 via-orange-600/10 to-transparent border border-amber-400/25 p-8">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black bg-gradient-to-r from-amber-400 to-orange-600 bg-clip-text text-transparent">
                  Compilação
                </h1>
              </div>
              <p className="text-foreground/60 text-sm max-w-md">As melhores cenas reunidas numa só produção.</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              placeholder="Pesquisar em Compilação..."
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
              <LayoutGrid size={28} className="text-foreground/20" />
            </div>
            <div className="text-center space-y-1">
              ...
              <p className="text-foreground/35 text-sm max-w-xs">
                Assim que um criador publicar um vídeo na categoria "Compilação", aparece aqui automaticamente.
              </p>
            </div>
