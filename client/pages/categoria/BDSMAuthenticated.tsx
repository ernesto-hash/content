// src/pages/categoria/BDSMAuthenticated.tsx
// Categoria: BDSM — slug BD: "bdsm"
...
export default function BDSMAuthenticatedPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (publishedVids.length === 0) {
      const info = `${vids.length} vídeo(s) com category~="bdsm" mas nenhum é published+public.\n${JSON.stringify(vids.map((v: any) => ({ status: v.status, visibility: v.visibility })), null, 2)}`;
      setDebugInfo((import.meta as any).env?.DEV ? info : null);
      setVideos([]);
      setLoading(false);
      return;
  ...
  useEffect(() => {
    const ch1 = supabase.channel("bdsm-auth-videos-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "videos" },
        (payload) => {
          const rec = (payload.new ?? payload.old) as any;
          if (!rec?.category || (rec.category as string).toLowerCase().trim() === "bdsm") fetchData();
        }
      ).subscribe();

    const ch2 = supabase.channel("bdsm-auth-likes-rt")
  ...
  return (
    <LayoutAuthenticated>
      <div className="max-container safe-area py-8 space-y-8">
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-400/20 via-slate-700/10 to-transparent border border-zinc-400/25 p-8">
          <div className="absolute -top-10 -right-10 w-48 h-48 bg-zinc-400/10 rounded-full blur-3xl" />
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-zinc-400 to-slate-700 flex items-center justify-center shadow-lg">
                  <Link2 size={20} className="text-white" />
                </div>
                <h1 className="text-3xl font-black bg-gradient-to-r from-zinc-400 to-slate-700 bg-clip-text text-transparent">
                  BDSM
                </h1>
              </div>
              <p className="text-foreground/60 text-sm max-w-md">Bondage, disciplina, dominação, submissão e muito mais.</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
              <Link2 size={28} className="text-foreground/20" />
            </div>
            <div className="text-center space-y-1">
              ...
              <p className="text-foreground/35 text-sm max-w-xs">
                Assim que um criador publicar um vídeo na categoria "BDSM", aparece aqui automaticamente.
              </p>
            </div>
