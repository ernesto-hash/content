// src/pages/categoria/Gay.tsx
// Categoria: Gay — slug BD: "gay"
...
export default function GayPage() {
  useDocumentTitle({ title: "Vídeos Gay - SuckOrSex" });
  ...
  const fetchData = useCallback(async () => {
    ...
    if (publishedVids.length === 0) {
      const info = `${vids.length} vídeo(s) com category~="gay" mas nenhum é published+public.\n${JSON.stringify(vids.map((v: any) => ({ status: v.status, visibility: v.visibility })), null, 2)}`;
      setDebugInfo((import.meta as any).env?.DEV ? info : null);
      setVideos([]);
      setLoading(false);
      return;
  ...
  useEffect(() => {
    const ch1 = supabase.channel("gay-videos-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "videos" },
        (payload) => {
          const rec = (payload.new ?? payload.old) as any;
          if (!rec?.category || (rec.category as string).toLowerCase().trim() === "gay") fetchData();
        }
      ).subscribe();

    const ch2 = supabase.channel("gay-likes-rt")
  ...
  return (
    <Layout>
      <div className="max-container safe-area py-8 space-y-8">
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-sky-400/20 via-indigo-600/10 to-transparent border border-sky-400/25 p-8">
          <div className="absolute -top-10 -right-10 w-48 h-48 bg-sky-400/10 rounded-full blur-3xl" />
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-400 to-indigo-600 flex items-center justify-center shadow-lg">
                  <Rainbow size={20} className="text-white" />
                </div>
                <h1 className="text-3xl font-black bg-gradient-to-r from-sky-400 to-indigo-600 bg-clip-text text-transparent">
                  Gay
                </h1>
              </div>
              <p className="text-foreground/60 text-sm max-w-md">Conteúdo masculino entre homens.</p>
            </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
              <Rainbow size={28} className="text-foreground/20" />
            </div>
            <div className="text-center space-y-1">
              ...
              <p className="text-foreground/35 text-sm max-w-xs">
                Assim que um criador publicar um vídeo na categoria "Gay", aparece aqui automaticamente.
              </p>
            </div>
