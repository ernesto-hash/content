// src/pages/categoria/EspanhaAuthenticated.tsx
// Categoria: Espanha — slug BD: "espanha"
...
export default function EspanhaAuthenticatedPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (publishedVids.length === 0) {
      const info = `${vids.length} vídeo(s) com category~="espanha" mas nenhum é published+public.\n${JSON.stringify(vids.map((v: any) => ({ status: v.status, visibility: v.visibility })), null, 2)}`;
      setDebugInfo((import.meta as any).env?.DEV ? info : null);
      setVideos([]);
      setLoading(false);
      return;
  ...
  useEffect(() => {
    const ch1 = supabase.channel("espanha-auth-videos-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "videos" },
        (payload) => {
          const rec = (payload.new ?? payload.old) as any;
          if (!rec?.category || (rec.category as string).toLowerCase().trim() === "espanha") fetchData();
        }
      ).subscribe();

    const ch2 = supabase.channel("espanha-auth-likes-rt")
  ...
  return (
    <LayoutAuthenticated>
      <div className="max-container safe-area py-8 space-y-8">
        ...
        {/* Hero */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-yellow-500/20 via-red-600/10 to-transparent border border-yellow-500/25 p-8">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black bg-gradient-to-r from-yellow-500 to-red-600 bg-clip-text text-transparent">
                  Espanha
                </h1>
              </div>
              <p className="text-foreground/60 text-sm max-w-md">Conteúdo espanhol com todo o seu charme.</p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="flex items-center gap-2 flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5">
            ...
            <input value={query} onChange={(e) => setQuery(e.target.value)}
              placeholder="Pesquisar em Espanha..."
          ...
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-foreground/35 text-sm max-w-xs">
                Assim que um criador publicar um vídeo na categoria "Espanha", aparece aqui automaticamente.
              </p>
            </div>
