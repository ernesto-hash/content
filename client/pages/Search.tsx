...
import { Search, Film, Users, Eye, Loader2, User, Play } from "lucide-react";
      ...
      <div className="max-container safe-area py-8">
        ...
        {noResults && (
          <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
            ...
            <Link to="/videos" className="mt-2 text-neon-pink text-sm hover:underline">
          ...
          <div className="space-y-10">
            ...
            {(tab === "tudo" || tab === "modelos") && models.length > 0 && (
              <section>
                ...
                <div className={`grid gap-3 ${tab === "modelos" ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8"}`}>
                  {models.map(m => (
                    <ModelCard key={m.id} m={m} basePath="/modelo" />
                  ))}
                </div>
              </section>
            )}
            ...
            {(tab === "tudo" || tab === "videos") && allVideos.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {allVideos.map(v => (
                    <VideoCard key={v.id} v={v} basePath="/video" />
                  ))}
                </div>
              </section>
            )}
