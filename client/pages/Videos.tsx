...
import {
  ...
  LayoutGrid, Hash, ChevronDown, VolumeX,
} from "lucide-react";
...
function VideoListCard({ video }: { video: Video }) {
  ...
  return (
    <Link to={`/video/${video.id}`}
      className="group flex gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/8 hover:bg-white/[0.06] hover:border-white/15 transition-all"
...
export default function VideosPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        if (!data) return;
        const unique = [...new Set((data as { category: string }[]).map(v => v.category))].sort();
  ...
  const fetchVideos = useCallback(async (pageNum: number, replace: boolean) => {
    ...
    let q = supabase
      .from("videos_enriched")
      .select("id, title, thumbnail_url, video_url, views, duration, created_at, category, likes_count", { count: "exact" })
      .eq("status", "published")
      .eq("visibility", "public")
      .range(from, to);
    ...
    if (selectedSort === "likes")    q = q.order("likes_count", { ascending: false });
